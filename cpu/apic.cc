/////////////////////////////////////////////////////////////////////////
// $Id: apic.cc,v 1.83.2.1 2006/05/20 19:07:56 vruppert Exp $
/////////////////////////////////////////////////////////////////////////
//
...
void bx_local_apic_c::register_state(bx_param_c *parent)
{
  ...
  new bx_shadow_num_c(parent, "base_addr", "", &base_addr, 16);
  new bx_shadow_num_c(parent, "id", "", &id, 16);
  new bx_shadow_num_c(parent, "spurious_vector", "", &spurious_vector, 16);
  new bx_shadow_bool_c(parent, "software_enabled", &software_enabled);
  new bx_shadow_bool_c(parent, "focus_disable", &focus_disable);
  new bx_shadow_num_c(parent, "task_priority", "", &task_priority, 16);
  new bx_shadow_num_c(parent, "log_dest", "", &log_dest, 16);
  new bx_shadow_num_c(parent, "dest_format", "", &dest_format, 16);
  bx_list_c *ISR = new bx_list_c(parent, "isr", BX_LOCAL_APIC_MAX_INTS);
  for (i=0; i<BX_LOCAL_APIC_MAX_INTS; i++) {
    sprintf(name, "0x%02x", i);
    new bx_shadow_num_c(ISR, strdup(name), "", &isr[i], 16);
  }
  bx_list_c *TMR = new bx_list_c(parent, "tmr", BX_LOCAL_APIC_MAX_INTS);
  for (i=0; i<BX_LOCAL_APIC_MAX_INTS; i++) {
    sprintf(name, "0x%02x", i);
    new bx_shadow_num_c(TMR, strdup(name), "", &tmr[i], 16);
  }
  bx_list_c *IRR = new bx_list_c(parent, "irr", BX_LOCAL_APIC_MAX_INTS);
  for (i=0; i<BX_LOCAL_APIC_MAX_INTS; i++) {
    sprintf(name, "0x%02x", i);
    new bx_shadow_num_c(IRR, strdup(name), "", &irr[i], 16);
  }
  new bx_shadow_num_c(parent, "error_status", "", &error_status, 16);
  new bx_shadow_num_c(parent, "shadow_error_status", "", &shadow_error_status, 16);
  new bx_shadow_num_c(parent, "icr_hi", "", &icr_hi, 16);
  new bx_shadow_num_c(parent, "icr_lo", "", &icr_lo, 16);
  bx_list_c *LVT = new bx_list_c(parent, "lvt");
  for (i=0; i<6; i++) {
    sprintf(name, "%d", i);
    new bx_shadow_num_c(LVT, strdup(name), "", &lvt[i], 16);
  }
  new bx_shadow_num_c(parent, "timer_initial", "", &timer_initial, 16);
  new bx_shadow_num_c(parent, "timer_current", "", &timer_current, 16);
  new bx_shadow_num_c(parent, "timer_divconf", "", &timer_divconf, 16);
  new bx_shadow_num_c(parent, "timer_divide_counter", "", &timer_divide_counter, 16);
  new bx_shadow_num_c(parent, "timer_divide_factor", "", &timer_divide_factor, 16);
  new bx_shadow_bool_c(parent, "timer_active", &timer_active);
  new bx_shadow_num_c(parent, "ticksInitial", "", &ticksInitial, 16);
